...
ser = serial.Serial('COM4', 9600)  # Reemplaza 'COM4' con el puerto de tu Arduino
...
# Función para establecer la velocidad del motor en FT232RL
def set_motor_speed(speed):
    ser.write(f'M{speed}\n'.encode())  # Envía el comando a la tarjeta FT232RL para ajustar la velocidad
...
motor_pin = 9  # Reemplaza con el pin de salida PWM de tu microcontrolador
...
def update_voltage_plot():
    voltage_plot.clear()
    voltage_plot.plot(range(len(voltage_values)), voltage_values, marker='o', linestyle='-', color='r')
    voltage_plot.set_xlabel('Muestras')
    voltage_plot.set_ylabel('Voltaje (V)')
    voltage_plot.set_title('Gráfico de Voltaje')
    voltage_canvas.draw()
...
def update_temperature_plot():
    ...
    temperature_plot.plot(range(len(temperature_values)), temperature_values, marker='o', linestyle='-', color='b')
    temperature_plot.set_xlabel('Muestras')
    temperature_plot.set_ylabel('Temperatura (°C)')
    temperature_plot.set_title('Gráfico de Temperatura')
